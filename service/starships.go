...
func starshipsHandler(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		count := 0
		for i := 1; ; i++ {
			item := database.GetValue("starships", strconv.Itoa(i))
			if len(item) != 0 {
				count++
				if count > pagelen*(page-1) {
					w.Write([]byte(item))
					if count >= pagelen*page || count >= database.GetBucketCount("starships") {
						break
					}
					w.Write([]byte(", \n"))
				}
...
func getStarshipsByID(w http.ResponseWriter, req *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	data := database.GetValue("starships", vars["id"])
	w.Write([]byte(data))
}

func starshipsPagesHandler(w http.ResponseWriter, req *http.Request) {
	data := database.GetBucketCount("starships")
